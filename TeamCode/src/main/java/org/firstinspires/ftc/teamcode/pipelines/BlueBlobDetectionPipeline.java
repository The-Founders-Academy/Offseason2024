...
public class BlueBlobDetectionPipeline extends OpenCvPipeline {
    ...
    public Mat processFrame(Mat input) {
        ...
        if (largestContour != null) {
            ...
            BluePropZone BlueZone = GetBluePropZone(cX);
        }
    ...
    private BluePropZone GetBluePropZone(double cord) {
        double coordinate = cord;
        double left_third = cameraWidth * 0.33;
        double center_third = cameraWidth * 0.66;

        BluePropZone BlueZone = BluePropZone.CENTER; // Sets initial value of BlueZone to Center



        if (coordinate <= left_third) {

           BlueZone = BluePropZone.LEFT;
        }
        else if (coordinate > left_third && coordinate <= center_third) {
            BlueZone = BluePropZone.CENTER;
        }
        else {
            BlueZone = BluePropZone.RIGHT;
