...
public class RedBlobDetectionPipeline extends OpenCvPipeline {
    ...
    public enum RedPropZone  {
        LEFT, CENTER, RIGHT
    }
    double cX = 0; // will display x coordinate of centroid
    double cY = 0; // will display y coordinate of centroid
    ...
    public Mat processFrame(Mat input) {
        ...
        if (largestContour != null) {
            ...
            RedPropZone RedZone = GetRedPropZone(cX);
        }
    ...
    private RedPropZone GetRedPropZone(double cord) {
        double coordinate = cord;
        double left_third = cameraWidth * 0.33;
        double center_third = cameraWidth * 0.66;

       RedPropZone RedZone = RedPropZone.CENTER; // Sets initial value of BlueZone to Center



        if (coordinate <= left_third) {

            RedZone = RedPropZone.LEFT;
        }
        else if (coordinate > left_third && coordinate <= center_third) {
            RedZone = RedPropZone.CENTER;
        }
        else {
            RedZone = RedPropZone.RIGHT;
